...
export class JsTreeDirective implements OnInit, OnDestroy, OnChanges {
    ...
    tree_init(op: any) {
        ...
        op.first_node_select =
            op.first_node_select === undefined ? true : false;
        this.op = op;
    ...
    tree_initUI() {
        ...
        const jstree_op = {
            ...
            types: {
                ...
                opened: {
                    icon:
                        'anticon anticon-folder-open icon-state-warning icon-lg',
                },
                default: {
                    icon: 'anticon anticon-folder icon-state-warning icon-lg',
                },
                    ...
                    .subscribe(
                        result => {
                            return self.$container
                                .jstree(true)
                                .set_text(node, res.value);
                        },
                        err => {
                            if (!err.message)
                                self.msgService.msg_error(
                                    '操作失败，请稍候再试！',
                                );
                            return void 0;
                        },
