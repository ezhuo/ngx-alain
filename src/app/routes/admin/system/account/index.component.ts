...
export class AccountComponent extends ParentIndexControl implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.searchSchema = {
      properties: {
        ...
        status: {
          type: 'string',
          title: '',
          enum: this.stateSrv.sysDicDicUnshift('account_status', { label: '请选择', value: '' }),
          default: '',
        }
      },
      ui: {}
    ...
    this.mainSchema = {
      ...
      required: ['login_username', 'login_pwd', 'login_pwd2', 'true_name', 'role_id'],
      ui: {
        spanLabel: 6,
        spanControl: 18,
        grid: {
    ...
    this.mainTableColumns = [
      ...
      { title: '联系地址', index: 'address' },
      {
        title: '状态', index: 'state_str', width: '100px', format: (item: any, col: any) => {
          return this.stateSrv.getSysDicDicLabel('account_status', item['status']);
        }
      },
      {
        title: '操作',
  ...
  treeDataSelectKeys = [];
  gettreeData() {
    this.___pageData$.company = this.httpSrv.get('/orginfo/tree').subscribe((result: any) => {
